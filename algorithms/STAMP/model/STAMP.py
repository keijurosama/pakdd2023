...
class Seq2SeqAttNN(NN):
    ...
    def __init__(self, model="stamp", dataset="rsc15_64", recsys_threshold_acc=0.68, cikm_threshold_acc=0.62,
        ...
        print("GPU:",tf.test.is_gpu_available(cuda_only=False, min_cuda_compute_capability=None)) # DBG
        #print("Num GPUs Available: ", len(tf.config.list_physical_devices('GPU'))) # DBG
    ...
    def build_model(self):
        ...
        self.w1 = tf.Variable(
            tf.random.normal([self.edim, self.edim], stddev=self.stddev), #DBG
            trainable=True
        )
