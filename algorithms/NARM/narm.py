...
class NARM:
    ...
    def fit(self, data, test=None):
        ...
        if test is not None:
            item_ids = list(dict.fromkeys(list(item_ids) + list(test[self.item_key].unique())))
            nis = len(item_ids)
        
        self.itemmap = pd.Series( index=item_ids, data=range(1,nis+1)  )
        # ---
    ...
    test_size=-1,  # If >0, we keep only this number of test example.
        ...
        (use_noise, x, mask,
         y, f_pred_prob, cost, attention) = self.build_model(tparams, model_options) # DBG access attention
         
        self.pred_function = f_pred_prob
        self.attention = attention # DBG access attention
    ...
    def build_model(self, tparams, options):
        ...
        f_weight = theano.function([x, mask], weight, name='f_weight') # DBG attention
        ...
        return use_noise, x, mask, y, f_pred_prob, cost, f_weight
